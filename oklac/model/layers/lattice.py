...
class LatticeLSTM(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, word_drop, word_alphabet_size, word_emb_dim, device, pretrain_word_emb=None,
        ...
        self.rnn = MultiInputLSTMCell(input_dim, hidden_dim).to(self.device)
        self.word_rnn = WordLSTMCell(word_emb_dim, hidden_dim).to(self.device)
        self.left2right = left2right
        self.word_dropout = self.word_dropout.to(self.device)
        self.word_emb = self.word_emb.to(self.device)
    ...
    def forward(self, input, skip_input_list, hidden=None):
        ...
        if hidden:
            ...
            hx = autograd.Variable(torch.zeros(batch_size, self.hidden_dim)).to(self.device)
            cx = autograd.Variable(torch.zeros(batch_size, self.hidden_dim)).to(self.device)
