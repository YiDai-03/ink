...
from ..test.predict_utils import get_entity,get_word
...
class Entity_Score(object):
    ...
    def result(self):
        ...
        for type,count in origin_counter.items():
            ...
            print("found=",found)
            print("right=",right)
            print("Type: %s - precision: %.4f - recall: %.4f - f1: %.4f"%(type,precision,recall,f1))
        total_recall, total_precision, total_f1 = self._compute(total_origin, total_found, total_right)
...
class Word_Score(object):
    ...
    def update(self,label_paths,pred_paths):
        ...
        for label_path,pre_path in zip(label_paths,pred_paths):
            label_words = get_word(label_path)
            pre_words = get_word(pre_path)
            self.origins.extend(label_words)
            self.founds.extend(pre_words)
           # print(label_path)
           # print("lab",label_words)
           # print("pre",pre_words)
            self.rights.extend([pre_entity for pre_entity in pre_words if pre_entity in label_words])
